...
function getBook(?array $columns = null): array
{
    if (!is_array($columns)) {
        $columns = [];
    }
    ...
    $authorizedColumns = ["id_user", "id_space", "book_date"];
    ...
    $getUserQuery = $databaseConnection->prepare("SELECT * FROM BOOKS WHERE $whereClause;");
