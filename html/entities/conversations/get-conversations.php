...
function getConversation(?array $columns = null): array
{
    if (!is_array($columns)) {
        $columns = [];
    }
    ...
    $authorizedColumns = ["id", "id_receiver", "send_date", "id_sender"];
    ...
    $getUserQuery = $databaseConnection->prepare("SELECT * FROM CONVERSATION WHERE $whereClause;");
