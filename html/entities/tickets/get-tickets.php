...
function getTicket(?array $columns = null): array
{
    if (!is_array($columns)) {
        $columns = [];
    }
    ...
    $authorizedColumns = ["id", "title", "description", "creation_date", "id_sender"];
    ...
    $getUserQuery = $databaseConnection->prepare("SELECT * FROM TICKET WHERE $whereClause;");
