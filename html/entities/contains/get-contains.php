...
function getContain(?array $columns = null): array
{
    if (!is_array($columns)) {
        $columns = [];
    }
    ...
    $authorizedColumns = ["id_article", "id_order", "quantity"];
    ...
    $getUserQuery = $databaseConnection->prepare("SELECT * FROM CONTAINS WHERE $whereClause;");
