...
function getSite(?array $columns = null): array
{
    if (!is_array($columns)) {
        $columns = [];
    }
    ...
    $authorizedColumns = ["id", "address", "postcode"];
    ...
    $getUserQuery = $databaseConnection->prepare("SELECT * FROM SITE WHERE $whereClause;");
