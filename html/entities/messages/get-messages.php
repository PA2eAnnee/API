...
function getMessage(?array $columns = null): array
{
    if (!is_array($columns)) {
        $columns = [];
    }
    ...
    $authorizedColumns = ["id", "id_sender", "content", "send_date", "id_conversation"];
    ...
    $getUserQuery = $databaseConnection->prepare("SELECT * FROM MESSAGE WHERE $whereClause;");
