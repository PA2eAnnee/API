...
function getArticle(?array $columns = null): array
{
    if (!is_array($columns)) {
        $columns = [];
    }
    ...
    $authorizedColumns = ["id", "description", "stock", "price"];
    $where = [];
    $sanitizedColumns = [];
    ...
    $getUserQuery = $databaseConnection->prepare("SELECT * FROM ARTICLES WHERE $whereClause;");
